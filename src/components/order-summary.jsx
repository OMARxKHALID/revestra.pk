...
const Row = ({ label, value, tone = "text-black/70" }) => (
  ...
  <>
    <ul className="divide-y divide-black/10">
      {items.map((item) => (
        ...
        >
          <div>
            <p className={cn(ITEM, "text-black")}>{item.name}</p>
            <p className={cn(META, "mt-1 text-black/45")}>
              {[item.size, item.condition].filter(Boolean).join(" · ")}
            </p>
          </div>
          <p className={cn(ITEM, "shrink-0 tabular-nums text-black")}>
    ...
    <p className={cn(TITLE, "flex items-baseline justify-between gap-6 border-t border-black/10 pt-5 text-black")}>
      <span>Total</span>
